...
jQuery(function($) {
    ...
    Context._init_ = function(dom, option, instance) {
        ...
        var _me = instance,//Context实例
            objn = 0;
        _me._jqDom = $(dom); //menu1

        //getting options
        if ( !! option) {
            _me._menuItems = _me._jqDom.find('[' + _me.dataProperty + ']');//data-menu
            ...
            _me.itemClass = option.itemClass; //menuItem
            _me.itemHover = option.itemHover; //active
        } else {
    ...
    $(document).on('scroll', function() {
        var scrollOffest = parseInt($(document).scrollTop()), //scrollTop() 方法设置或返回被选元素的【垂直滚动条位置】。
            scrollDir = scrollOffest > lastScrollTop ? 'down' : 'up';

        for (var i in _ctxList) {
            _ctxList[i].onScroll(scrollDir, scrollOffest);//_ctxList[""] 为 Context实例
        }
